(function() {
  ...
  _.each = function(collection, iterator) {
    if(Array.isArray(collection)){
      ...
      for (var ele in collection){
        iterator(collection[ele], ele, collection);
      }
    }
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var toBeReduced = collection
    if (accumulator=== null || accumulator === undefined){
      accumulator = collection[0];
      toBeReduced = toBeReduced.slice(1, collection.length);
    }
    _.each(toBeReduced, function(value){
  ...
  _.delay = function(func, wait) {
    ...
    if (arguments.length>2){
      realArguments = realArguments.slice(2, arguments.length);
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 0; i<array.length; i++){
      var truthCounter = true;
      while(truthCounter){
        var randomNumber= Math.floor((Math.random() * array.length));
        if(randomArray[randomNumber] === undefined){
          randomArray[randomNumber] = array[i];
          truthCounter=false;
        }
      }
    }
